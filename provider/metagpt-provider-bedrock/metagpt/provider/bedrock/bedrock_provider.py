...
class CohereProvider(BaseBedrockProvider):
    ...
    def _is_command_r_model(self) -> bool:
        """Check if this is a Command-R model (vs standard Command model)"""
        return "command-r" in self.model_name.lower()

    def _get_completion_from_dict(self, rsp_dict: dict) -> str:
        return rsp_dict["generations"][0]["text"]

    def messages_to_prompt(self, messages: list[dict]) -> str:
        if self._is_command_r_model():
...
class Ai21Provider(BaseBedrockProvider):
    ...
    def _get_completion_from_dict(self, rsp_dict: dict) -> str:
        if self.model_type == "j2":
            ...
            # Handle different response formats in test vs real environment
            if "choices" in rsp_dict:
                return rsp_dict["choices"][0]["message"]["content"]
            # For tests, we might have J2 format with completions field
            elif "completions" in rsp_dict:
                return rsp_dict["completions"][0]["data"]["text"]
            else:
                raise ValueError(f"Unexpected response format for AI21 Jamba model: {rsp_dict}")
