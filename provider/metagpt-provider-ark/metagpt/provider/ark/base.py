...
class ArkLLM(OpenAILLM):
    ...
    def _update_costs(self, usage: Union[dict, BaseModel], model: str = None, local_calc_usage: bool = True):
        # Only update token costs in cost_manager if it exists (created by parent)
        if self.cost_manager:
            ...
            if model and model in self.cost_manager.token_costs:
                self.pricing_plan = model

        # Let the parent class handle the actual cost updating logic
